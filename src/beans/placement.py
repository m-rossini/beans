...
class ClusteredPlacementStrategy(PlacementStrategy):
    ...
    def place(self, count: int, width: int, height: int, size: int) -> List[Tuple[float, float]]:
        ...
        centers = [(_snap_to_half_pixel(random.uniform(0, width)), _snap_to_half_pixel(random.uniform(0, height))) for _ in range(clusters)]
        ...
        for i in range(count):
            ...
            x = _snap_to_half_pixel(x)
            y = _snap_to_half_pixel(y)
            positions.append((x, y))
        logger.info(f">>>>> Generated {len(positions)} positions")
        return positions
