...
class BeansConfig:
    ...
    min_energy_efficiency: float = (
        0.3  # Minimum energy efficiency at extreme ages (0-1).
    )
    ...
    fat_gain_rate: float = (
        0.02  # Rate of fat storage from surplus energy (fraction per step).
    )
    fat_burn_rate: float = (
        0.02  # Rate of fat burning from energy deficit (fraction per step).
    )
    # Survival system configuration
    starvation_base_depletion: float = (
        1.0  # Base size units consumed per starvation tick
    )
    starvation_depletion_multiplier: float = (
        1.0  # Multiplier applied when energy is <= 0
    )
    enable_obesity_death: bool = False  # Toggle probabilistic obesity death
    obesity_death_probability: float = (
        0.0  # Probability of death when above obesity threshold
    )
    obesity_threshold_factor: float = (
        1.0  # Threshold relative to max_bean_size to consider obese
    )
    metabolism_base_burn: float = (
        0.01  # Basal metabolism burn rate per tick (energy units).
    )
    ...
    size_penalty_above_k: float = (
        0.20  # Speed penalty factor when bean is overweight (z > 0).
    )
    size_penalty_below_k: float = (
        0.15  # Speed penalty factor when bean is underweight (z < 0).
    )
    ...
    energy_loss_on_bounce: float = (
        2.0  # Energy lost when bean bounces off a wall or obstacle.
    )
...
def load_config(
...
) -> tuple[WorldConfig, BeansConfig, EnvironmentConfig]:
    ...
    world_config = WorldConfig(
        male_sprite_color=world_data.get(
            "male_sprite_color", DEFAULT_WORLD_CONFIG.male_sprite_color
        ),
        female_sprite_color=world_data.get(
            "female_sprite_color", DEFAULT_WORLD_CONFIG.female_sprite_color
        ),
        male_female_ratio=world_data.get(
            "male_female_ratio", DEFAULT_WORLD_CONFIG.male_female_ratio
        ),
        width=world_data.get("width", DEFAULT_WORLD_CONFIG.width),
        height=world_data.get("height", DEFAULT_WORLD_CONFIG.height),
        population_density=world_data.get(
            "population_density", DEFAULT_WORLD_CONFIG.population_density
        ),
        placement_strategy=world_data.get(
            "placement_strategy", DEFAULT_WORLD_CONFIG.placement_strategy
        ),
        population_estimator=world_data.get(
            "population_estimator", DEFAULT_WORLD_CONFIG.population_estimator
        ),
        background_color=world_data.get(
            "background_color", DEFAULT_WORLD_CONFIG.background_color
        ),
        max_age_years=world_data.get(
            "max_age_years", DEFAULT_WORLD_CONFIG.max_age_years
        ),
        rounds_per_year=world_data.get(
            "rounds_per_year", DEFAULT_WORLD_CONFIG.rounds_per_year
        ),
    ...
    beans_config = BeansConfig(
        ...
        initial_energy=beans_data.get(
            "initial_energy", DEFAULT_BEANS_CONFIG.initial_energy
        ),
        energy_gain_per_step=beans_data.get(
            "energy_gain_per_step", DEFAULT_BEANS_CONFIG.energy_gain_per_step
        ),
        energy_cost_per_speed=beans_data.get(
            "energy_cost_per_speed", DEFAULT_BEANS_CONFIG.energy_cost_per_speed
        ),
        min_energy_efficiency=beans_data.get(
            "min_energy_efficiency", DEFAULT_BEANS_CONFIG.min_energy_efficiency
        ),
        min_speed_factor=beans_data.get(
            "min_speed_factor", DEFAULT_BEANS_CONFIG.min_speed_factor
        ),
        initial_bean_size=beans_data.get(
            "initial_bean_size", DEFAULT_BEANS_CONFIG.initial_bean_size
        ),
        min_bean_size=beans_data.get(
            "min_bean_size", DEFAULT_BEANS_CONFIG.min_bean_size
        ),
        base_bean_size=beans_data.get(
            "base_bean_size", DEFAULT_BEANS_CONFIG.base_bean_size
        ),
        max_bean_size=beans_data.get(
            "max_bean_size", DEFAULT_BEANS_CONFIG.max_bean_size
        ),
        energy_baseline=beans_data.get(
            "energy_baseline", DEFAULT_BEANS_CONFIG.energy_baseline
        ),
        male_bean_color=beans_data.get(
            "male_bean_color", DEFAULT_BEANS_CONFIG.male_bean_color
        ),
        female_bean_color=beans_data.get(
            "female_bean_color", DEFAULT_BEANS_CONFIG.female_bean_color
        ),
        fat_gain_rate=beans_data.get(
            "fat_gain_rate", DEFAULT_BEANS_CONFIG.fat_gain_rate
        ),
        fat_burn_rate=beans_data.get(
            "fat_burn_rate", DEFAULT_BEANS_CONFIG.fat_burn_rate
        ),
        # Survival system fields
        starvation_base_depletion=beans_data.get(
            "starvation_base_depletion", DEFAULT_BEANS_CONFIG.starvation_base_depletion
        ),
        starvation_depletion_multiplier=beans_data.get(
            "starvation_depletion_multiplier",
            DEFAULT_BEANS_CONFIG.starvation_depletion_multiplier,
        ),
        enable_obesity_death=beans_data.get(
            "enable_obesity_death", DEFAULT_BEANS_CONFIG.enable_obesity_death
        ),
        obesity_death_probability=beans_data.get(
            "obesity_death_probability", DEFAULT_BEANS_CONFIG.obesity_death_probability
        ),
        obesity_threshold_factor=beans_data.get(
            "obesity_threshold_factor", DEFAULT_BEANS_CONFIG.obesity_threshold_factor
        ),
        metabolism_base_burn=beans_data.get(
            "metabolism_base_burn", DEFAULT_BEANS_CONFIG.metabolism_base_burn
        ),
        energy_to_fat_ratio=beans_data.get(
            "energy_to_fat_ratio", DEFAULT_BEANS_CONFIG.energy_to_fat_ratio
        ),
        fat_to_energy_ratio=beans_data.get(
            "fat_to_energy_ratio", DEFAULT_BEANS_CONFIG.fat_to_energy_ratio
        ),
        energy_max_storage=beans_data.get(
            "energy_max_storage", DEFAULT_BEANS_CONFIG.energy_max_storage
        ),
        size_sigma_frac=beans_data.get(
            "size_sigma_frac", DEFAULT_BEANS_CONFIG.size_sigma_frac
        ),
        size_penalty_above_k=beans_data.get(
            "size_penalty_above_k", DEFAULT_BEANS_CONFIG.size_penalty_above_k
        ),
        size_penalty_below_k=beans_data.get(
            "size_penalty_below_k", DEFAULT_BEANS_CONFIG.size_penalty_below_k
        ),
        size_penalty_min_above=beans_data.get(
            "size_penalty_min_above", DEFAULT_BEANS_CONFIG.size_penalty_min_above
        ),
        size_penalty_min_below=beans_data.get(
            "size_penalty_min_below", DEFAULT_BEANS_CONFIG.size_penalty_min_below
        ),
        pixels_per_unit_speed=beans_data.get(
            "pixels_per_unit_speed", DEFAULT_BEANS_CONFIG.pixels_per_unit_speed
        ),
        collision_enable=beans_data.get(
            "collision_enable", DEFAULT_BEANS_CONFIG.collision_enable
        ),
        collision_base_damage=beans_data.get(
            "collision_base_damage", DEFAULT_BEANS_CONFIG.collision_base_damage
        ),
        collision_damage_speed_factor=beans_data.get(
            "collision_damage_speed_factor",
            DEFAULT_BEANS_CONFIG.collision_damage_speed_factor,
        ),
        collision_min_damage=beans_data.get(
            "collision_min_damage", DEFAULT_BEANS_CONFIG.collision_min_damage
        ),
        collision_damage_size_exponent=beans_data.get(
            "collision_damage_size_exponent",
            DEFAULT_BEANS_CONFIG.collision_damage_size_exponent,
        ),
        collision_damage_sex_factors=tuple(
            beans_data.get(
                "collision_damage_sex_factors",
                DEFAULT_BEANS_CONFIG.collision_damage_sex_factors,
            )
        ),
        energy_loss_on_bounce=beans_data.get(
            "energy_loss_on_bounce", DEFAULT_BEANS_CONFIG.energy_loss_on_bounce
        ),
    )
    ...
    environment_config: EnvironmentConfig = EnvironmentConfig(
        ...
        decomposition_fraction_to_food=env_data.get(
            "decomposition_fraction_to_food", 0.5
        ),
        ...
        temperature_migration_vector=tuple(
            env_data.get("temperature_migration_vector", (0.0, 0.0))
        ),
    ...
    def validate_world(cfg: WorldConfig) -> None:
        ...
        if cfg.population_density <= 0:
            raise ValueError(
                f"Population density must be >= 0, got {cfg.population_density}"
            )
        if cfg.male_female_ratio <= 0:
            raise ValueError(
                f"Male/female ratio must be > 0, got {cfg.male_female_ratio}"
            )
    ...
    def validate_beans(cfg: BeansConfig) -> None:
        if cfg.speed_min > cfg.speed_max:
            raise ValueError(
                f"speed_min ({cfg.speed_min}) cannot be greater than speed_max ({cfg.speed_max})"
            )
        # Speeds cannot be zero, as zero speed is not allowed
        if cfg.speed_min == 0 or cfg.speed_max == 0:
            raise ValueError(
                f"speed_min and speed_max must be non-zero, got speed_min={cfg.speed_min}, speed_max={cfg.speed_max}"
            )
        if cfg.initial_energy < 0:
            raise ValueError(f"initial_energy must be >= 0, got {cfg.initial_energy}")
        if cfg.energy_gain_per_step < 0:
            raise ValueError(
                f"energy_gain_per_step must be >= 0, got {cfg.energy_gain_per_step}"
            )
        if cfg.energy_cost_per_speed < 0:
            raise ValueError(
                f"energy_cost_per_speed must be >= 0, got {cfg.energy_cost_per_speed}"
            )
        if cfg.initial_bean_size <= 0:
            raise ValueError(
                f"initial_bean_size must be > 0, got {cfg.initial_bean_size}"
            )
        if cfg.pixels_per_unit_speed <= 0:
            raise ValueError(
                f"pixels_per_unit_speed must be > 0, got {cfg.pixels_per_unit_speed}"
            )
        if cfg.energy_loss_on_bounce < 0:
            raise ValueError(
                f"energy_loss_on_bounce must be >= 0, got {cfg.energy_loss_on_bounce}"
            )
        if not (0.0 <= cfg.min_speed_factor <= 1.0):
            raise ValueError(
                f"min_speed_factor must be between 0.0 and 1.0, got {cfg.min_speed_factor}"
            )
        # Validate survival configuration
        if cfg.starvation_base_depletion < 0.0:
            raise ValueError(
                f"starvation_base_depletion must be >= 0.0, got {cfg.starvation_base_depletion}"
            )
        if cfg.starvation_depletion_multiplier < 0.0:
            raise ValueError(
                f"starvation_depletion_multiplier must be >= 0.0, got {cfg.starvation_depletion_multiplier}"
            )
        if not (0.0 <= cfg.obesity_death_probability <= 1.0):
            raise ValueError(
                f"obesity_death_probability must be between 0.0 and 1.0, got {cfg.obesity_death_probability}"
            )
        if cfg.obesity_threshold_factor <= 0.0:
            raise ValueError(
                f"obesity_threshold_factor must be > 0.0, got {cfg.obesity_threshold_factor}"
            )
        ...
        try:
            ...
            raise ValueError(
                f"collision_damage_sex_factors must be a sequence of two numbers, got {cfg.collision_damage_sex_factors}"
            )
        if len(factors) != 2:
            raise ValueError(
                f"collision_damage_sex_factors must contain exactly two values (female, male), got {factors}"
            )
        for v in factors:
            if not isinstance(v, (int, float)):
                raise ValueError(
                    f"collision_damage_sex_factors values must be numeric, got {factors}"
                )
            if v < 0.0:
                raise ValueError(
                    f"collision_damage_sex_factors values must be >= 0.0, got {factors}"
                )
    ...
    def validate_environment(cfg: EnvironmentConfig) -> None:
        ...
        if not (0.0 <= cfg.decomposition_fraction_to_food <= 1.0):
            raise ValueError(
                f"decomposition_fraction_to_food must be between 0.0 and 1.0, got {cfg.decomposition_fraction_to_food}"
            )
    ...
    logger.debug(
        f">>>>> Environment config validation passed, EnvironmentConfig: {environment_config}"
    )
