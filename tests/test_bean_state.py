...
def test_beanstate_init_and_reset():
    ...
    state = BeanState(id=1, age=2.0, speed=3.0, energy=4.0, size=5.0, alive=True)
    assert state.id == 1
...
def test_beanstate_id_is_immutable():
    state = BeanState(id=1, age=0.0, speed=0.0, energy=0.0, size=1.0, alive=True)
    with pytest.raises(AttributeError):
        state.id = 2


def test_beanstate_partial_store_updates_only_informed_fields():
    state = BeanState(id=10, age=1.0, speed=2.0, energy=3.0, size=4.0, alive=True)
